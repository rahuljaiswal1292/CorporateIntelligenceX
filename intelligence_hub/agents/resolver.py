...
class ResolverAgent:
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        if exchange_url:
            logs.append(
                f"Resolver: Found Exchange URL: {exchange_url} ({exchange_name})"
            )
        if wiki_url:
        ...
        return {
            ...
            "logs": logs
            + [
                f"Resolved '{query}' to {final_ticker} ({exchange_name}) via Web Search & Playwright."
            ],
        }
