...
class WebScraperConnector:
    """
    Connector for Web Scraping APIs (ScraperAPI, Scrape.do).
    ...
    def __init__(self, api_key: str = None, provider: str = "scraperapi"):
        ...
        Args:
            ...
            provider: Which provider to use ('scraperapi', 'scrape.do')
        """
        ...
        if self.api_key:
            ...
            if PlaywrightConnector:
                ...
                logger.warning(
                    "SCRAPER_API_KEY not found. Running in LOCAL PLAYWRIGHT MODE."
                )
            else:
                self.mode = "MOCK"
                logger.warning(
                    "SCRAPER_API_KEY not found and Playwright missing. Running in MOCK MODE."
                )
    ...
    def _scrape_live(
        ...
        try:
            ...
            logger.error(
                f"Web Scraper Error {response.status_code}: {response.text[:500]}"
            )

            # Fallback
            if response.status_code in [404, 401, 403, 429, 500]:
                logger.info(
                    f"API Error {response.status_code}. Switching to Playwright Fallback..."
                )
    ...
    def _scrape_playwright_sync(
        self, url: str, wait_for: str, js_scenario: dict
    ) -> str:
        ...
        try:
            ...
            async def runner():
                ...
                try:
                    content, page = await connector.scrape(
                        url, wait_for_selector=wait_for, js_scenario=js_scenario
                    )
                    return content
                finally:
        ...
        try:
            ...
            if self.mode == "LIVE" and self.api_key:
                ...
                if r.status_code == 200:
                    ...
                    if not is_valid:
                        logger.warning(
                            f"Content-Type mismatch: got '{content_type}', expected '{expected_type}'"
                        )

                    return (r.content, content_type, is_valid)

                logger.warning(
                    f"Web Scraper download failed {r.status_code}, attempting direct fallback..."
                )
    ...
    def _build_download_params(self, url: str) -> dict:
        ...
        # Default
        return {"api_key": self.api_key, "url": url, "render": "false"}

    def _validate_content_type(
        self, content_type: str, expected_type: str = None
    ) -> bool:
        """
