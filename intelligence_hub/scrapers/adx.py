...
class ADXScraper:
    ...
    def __init__(self, connector: WebScraperConnector = None, max_age_years: int = 3):
        ...
        self.download_manager = (
            DownloadManager(max_age_years=max_age_years) if DownloadManager else None
        )
            ...
            async def process_page(url, page_type):
                ...
                try:
                    ...
                    if page_type == "overview":
                        ...
                        extracted_data = await self._interact_and_extract_financials(
                            page, ticker, page_type
                        )
                    elif page_type in [
                        "disclosures",
    ...
    async def _extract_overview(self, page: Page, ticker: str, page_type: str) -> dict:
        ...
        try:
            ...
            await page.wait_for_selector(
                ".adx-profile_details-listedHeader-left-details", timeout=10000
            )
        except:
            pass
        ...
        if header:
            ...
            if h2:
                name_candidates.append(h2.get_text(strip=True))
            h4 = header.select_one("h4")
            if h4:
                name_candidates.append(h4.get_text(strip=True))
        ...
        if profile["company_name"] == "NOT AVAILABLE":
            ...
            if h1:
                ...
                profile["company_name"] = (
                    ...
                    .replace("Company Profile Overview", "")
                    .strip(" | ADX")
                )
    ...
    async def _interact_and_extract_financials(
        ...
        try:
            buttons = (
                ...
                .filter(has_text=re.compile(r"annual|quarterly|interim", re.IGNORECASE))
                .all()
            )
    ...
    async def _extract_disclosures(
        self, page: Page, ticker: str, page_type: str
    ) -> dict:
