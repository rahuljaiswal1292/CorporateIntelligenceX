...
class DFMDownloadManager:
    ...
    async def download_with_retry(
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(f"DFM Download attempt {attempt + 1}/{max_retries}: {doc_info.get('text', 'Unknown')[:50]}")
                
                # Attempt download with dual strategy
                result = await self._download_with_dual_strategy(element, page, doc_info, target_dir)
                
                if result:
                    logger.info(f"✓ DFM Download successful: {os.path.basename(result)}")
        ...
        logger.error(f"✗ DFM Download failed after {max_retries} attempts: {doc_info.get('url', 'Unknown')}")
    ...
    async def _download_with_dual_strategy(
        ...
        if url and (url.startswith('http') or url.startswith('/') or 'feeds.dfm.ae' in url):
        ...
        if element:
            try:
                ...
                if not suggested_filename or len(suggested_filename) < 5:
                    suggested_filename = self._generate_filename(unquote(download.url), doc_info, "")
                
                if not suggested_filename.lower().endswith(('.pdf', '.xlsx', '.xls', '.docx', '.doc')):
                    # Check expected type
    ...
    async def _download_from_url(self, page: Page, url: str, doc_info: Dict, target_dir: str) -> Optional[str]:
        ...
        try:
            ...
            if not self._validate_content_bytes(content, doc_info): return None
            ...
            if not any(filename.lower().endswith(ext) for ext in ['.pdf', '.xlsx', '.xls', '.doc', '.docx']):
                ext = doc_info.get('expected_type', 'pdf')
                if '.' not in filename: filename += f".{ext}"
    ...
    def _validate_content_bytes(self, content: bytes, doc_info: Dict) -> bool:
        if len(content) < self.min_file_size: return False
    ...
    async def _ensure_valid_excel(self, filepath: str) -> str:
        ...
        try:
            ...
            if b'<html' in content_start.lower() and b'urn:schemas-microsoft-com:office:excel' in content_start.lower():
                logger.info(f"detected HTML content in Excel file {os.path.basename(filepath)}. Converting to XLSX...")
                ...
                try:
                    ...
                    if dfs:
                        ...
                        logger.info(f"Converted to real Excel: {os.path.basename(new_path)}")
                    ...
                    logger.warning(f"Failed to convert HTML-XLS to XLSX: {e}. Keeping original.")
                    return filepath
    ...
    async def _extract_document_date(self, filepath: str, doc_info: Dict) -> Optional[datetime]:
        ...
        text_context = f"{doc_info.get('text', '')} {doc_info.get('title', '')} {filename}"
        
        # DFM Specific patterns (often DD MM YYYY)
        dfm_pattern = re.search(r'(\d{2})[\s\-_\.](\d{2})[\s\-_\.](\d{4})', text_context)
        if dfm_pattern:
            try:
                return datetime(int(dfm_pattern.group(3)), int(dfm_pattern.group(2)), int(dfm_pattern.group(1)))
            except: pass
    ...
    def _generate_filename(self, url: str, doc_info: Dict, content_type: str) -> str:
        ...
        ext = 'pdf'
        if 'excel' in content_type: ext = 'xlsx'
    ...
    def _resolve_duplicate(self, new_file: str, existing_file: str, file_hash: str) -> str:
        if os.path.abspath(new_file) == os.path.abspath(existing_file): return new_file
    ...
    async def download_batch_parallel(self, page: Page, documents: List[Dict], target_dir: str, max_concurrent: int = 5) -> List[Optional[str]]:
        ...
        async def sem_download(doc, idx):
            async with semaphore:
                ...
                return await self.download_with_retry(link if link and await link.count() > 0 else None, page, doc, target_dir)
